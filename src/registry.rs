...
/// the `MOBILITYDB_SHIM_WASM` env var.
...
pub fn load_shim() -> Result<()> {
    ...
    let path = std::env::var("MOBILITYDB_SHIM_WASM").with_context(|| {
        format!("Set MOBILITYDB_SHIM_WASM=/path/to/composed-shim.wasm before .load")
    })?;
    let ext =
