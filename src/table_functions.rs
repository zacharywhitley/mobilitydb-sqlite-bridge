...
pub fn register_all(conn: &Connection) -> Result<()> {
    register_udtf(conn, "as_of_join_bool")?;
    register_udtf(conn, "as_of_join_float")?;
    register_udtf(conn, "as_of_join_int")?;
    register_udtf(conn, "as_of_join_text")?;
    register_udtf(conn, "interval_tree_query_overlapping")?;
    register_udtf(conn, "interval_tree_query_point")?;
    register_udtf(conn, "kdtree_xy_nearest_k")?;
    register_udtf(conn, "kdtree_xy_within")?;
    register_udtf(conn, "kdtree_xyz_nearest_k")?;
    register_udtf(conn, "kdtree_xyz_within")?;
    register_udtf(conn, "nearest_join_bool")?;
    register_udtf(conn, "nearest_join_float")?;
    register_udtf(conn, "nearest_join_int")?;
    register_udtf(conn, "nearest_join_text")?;
    register_udtf(conn, "octree_query_box")?;
    register_udtf(conn, "octree_query_sphere")?;
    register_udtf(conn, "quadtree_query_box")?;
    register_udtf(conn, "sequence_json")?;
    register_udtf(conn, "spatial_index_find_in_envelope")?;
    register_udtf(conn, "spatial_index_find_within_distance")?;
    register_udtf(conn, "spatial_index_nearest")?;
    register_udtf(conn, "stindex_nearest")?; // alias of spatial_index_nearest
    register_udtf(conn, "spatial_index_nearest_k")?;
    register_udtf(conn, "stindex_nearest_k")?; // alias of spatial_index_nearest_k
    register_udtf(conn, "stindex_find_in_period")?;
    register_udtf(conn, "stindex_find_in_spatial")?;
    register_udtf(conn, "stindex_find_in_stbox")?;
    register_udtf(conn, "tcbuffer_to_rows")?;
    register_udtf(conn, "temporal_index_find_after")?;
    register_udtf(conn, "temporal_index_find_at")?;
    register_udtf(conn, "temporal_index_find_before")?;
    register_udtf(conn, "temporal_index_find_in_range")?;
    register_udtf(conn, "temporal_index_find_overlapping")?;
    register_udtf(conn, "temporal_join_bool")?;
    register_udtf(conn, "temporal_join_float")?;
    register_udtf(conn, "temporal_join_int")?;
    register_udtf(conn, "temporal_join_text")?;
    register_udtf(conn, "tfloat_time_split")?;
    register_udtf(conn, "tfloat_value_split")?;
    register_udtf(conn, "tgeogpoint_to_rows")?;
    register_udtf(conn, "tgeompoint3d_to_rows")?;
    register_udtf(conn, "tgeompoint_to_rows")?;
    register_udtf(conn, "tint_time_split")?;
    register_udtf(conn, "tint_value_split")?;
    register_udtf(conn, "tpose_to_rows")?;
    // Phase 4c: 43 canonical + 2 alias UDTFs registered.
    Ok(())
}
